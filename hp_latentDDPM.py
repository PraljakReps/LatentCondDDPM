...
def get_args():
    ...
    parser.add_argument('-zi', '--z_init', default = 1, help = 'flag: init latent space dimension', type = int)
    parser.add_argument('-zf', '--z_final', default = 10, help = 'flag: final latent space dimension', type = int)
...
if __name__ == '__main__':
    ...
    z_init = args.z_init # initial latent space hp
    z_final = args.z_final # final latent space hp
    ...
    # prepare data
    train_dataloader, test_dataloader = prepare_data(batch_size=batch_size)
    

    hp_hist_dict = {
            }
    for z_dim in range(z_init, z_final):
        # train model
        ...
        hp_hist_dict[f'train_loss[z_dim={z_dim}]'] = hist_dict['train_loss']
        hp_hist_dict[f'test_loss[z_dim={z_dim}]'] = hist_dict['test_loss']
        hp_hist_dict[f'train_mse_loss[z_dim={z_dim}]'] = hist_dict['train_mse_loss']
        hp_hist_dict[f'test_mse_loss[z_dim={z_dim}]'] = hist_dict['test_mse_loss']
        hp_hist_dict[f'train_kld_loss[z_dim={z_dim}]'] = hist_dict['train_kld_loss']
        hp_hist_dict[f'test_kld_loss[z_dim={z_dim}]'] = hist_dict['test_kld_loss']
        hp_hist_dict[f'train_mmd_loss[z_dim={z_dim}]'] = hist_dict['train_mmd_loss']
        hp_hist_dict[f'test_mmd_loss[z_dim={z_dim}]'] = hist_dict['test_mmd_loss']
            

        # save model
        torch.save(model.state_dict(), f'{output_model_path}[z_dim={z_dim}].pth')


    # dataframe of the history dict
    hp_hist_df = pd.DataFrame(hp_hist_dict)

    # save dataframe
    hp_hist_df.to_csv(f'{output_csv_path}', index = False)
